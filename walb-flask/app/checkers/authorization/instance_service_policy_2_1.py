...
class InstanceServicePolicyChecker(BaseChecker):
        ...
        try:
            ...
            try:
                ...
                return {
                    ...
                    'recommendation': "인스턴스 관련 서비스에 과도한 권한(*FullAccess)이 부여되지 않도록 최소 권한 원칙을 적용하세요."
                }
    ...
    def _format_result_summary(self, result):
        ...
        if findings_count > 0:
            return f"[⚠ WARNING] 2.1 인스턴스 관련 서비스에 과도한 권한이 부여되었습니다 ({findings_count}건)."
        else:
            return "[✓ COMPLIANT] 2.1 인스턴스 관련 서비스에 과도한 권한이 부여된 주체가 없습니다."
    ...
    def _get_fix_options(self, result):
        """자동 조치 옵션 반환"""
        # 원본 2.1은 권한 변경이 위험하므로 자동 조치 없음
        return None
    
    def _get_manual_guide(self, result):
        """수동 조치 가이드 반환 - 원본 2.1 fix() 함수 내용"""
        ...
        return {
            'title': '2.1 인스턴스 서비스 정책 수동 조치 가이드',
    ...
    def execute_fix(self, selected_items):
        """자동 조치 실행"""
        # 원본 2.1은 권한 변경이 위험하므로 수동 조치 가이드만 제공
        return [{
            'item': 'manual_fix_only',
            'status': 'info',
            'message': '[FIX] 2.1 과도한 권한 정책 조치는 운영에 큰 영향을 줄 수 있어 자동화되지 않습니다.'
        }, {
            'item': 'fix_guide_1',
            'status': 'info',
