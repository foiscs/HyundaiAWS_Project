"""
2.2 네트워크 서비스 정책 관리 진단 (Flask용)
mainHub의 network_service_policy_2_2.py를 Streamlit 종속성 제거하여 이식
"""
import boto3
...
class NetworkServicePolicyChecker(BaseChecker):
    ...
    def run_diagnosis(self):
        """
        진단 수행
        - 주요 네트워크 서비스에 대해 과도한 권한(*FullAccess)이 부여되었는지 점검
        """
        try:
            if self.session:
            ...
            return {
                "status": "error",
                "error_message": f"네트워크 서비스 정책 정보를 가져오는 중 오류 발생: {str(e)}"
            }
    ...
    def _format_result_summary(self, result):
        ...
        if finding_count > 0:
            return f"⚠️ 네트워크 서비스에서 {finding_count}개의 과도한 권한이 발견되었습니다. (영향 서비스: {len(services_affected)}개)"
        else:
            return "✅ 네트워크 서비스에 과도한 권한이 부여된 주체가 없습니다."
    ...
    def _format_result_details(self, result):
        ...
        details = {
            "네트워크 권한 검사 결과": {
                ...
                "영향받는 네트워크 서비스": result.get('services_affected', []),
                "recommendation": "네트워크 서비스 FullAccess 정책 대신 업무에 필요한 최소 권한만 부여하세요."
            }
        }
        
        if result.get('findings'):
            details["과도한 네트워크 권한 주체"] = {}
            for finding in result['findings']:
                entity_key = f"{finding['type']}: {finding['name']}"
                details["과도한 네트워크 권한 주체"][entity_key] = {
                    "정책": finding['policy'],
                    "네트워크 서비스": finding['service'],
    ...
    def _get_fix_options(self, result):
        ...
        return [{
            "type": "manual_network_review",
            "title": "네트워크 권한 수동 검토",
            "description": "네트워크 권한 변경은 서비스 연결에 영향을 줄 수 있어 수동 검토가 필요합니다.",
    ...
    def execute_fix(self, selected_items):
        ...
        return [{
            ...
            "message": "네트워크 권한 변경은 서비스 연결에 영향을 줄 수 있어 수동 조치만 가능합니다."
        }]