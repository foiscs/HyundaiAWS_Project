...
def check():
    ...
    - 주요 네트워크 서비스에 대해 과도한 권한(*FullAccess)이 부여되었는지 점검하고, 해당 내역을 반환
    """
    ...
    try:
        ...
        if not findings:
            print("[✓ COMPLIANT] 2.2 네트워크 관련 서비스에 과도한 권한이 부여된 주체가 없습니다.")
        else:
            print(f"[⚠ WARNING] 2.2 네트워크 관련 서비스에 과도한 권한이 부여되었습니다 ({len(findings)}건).")
...
def fix(findings):
    """
    [2.2] 네트워크 서비스 정책 관리 조치
    - 권한 변경은 위험하므로 자동 조치 대신 수동 조치 가이드를 제공
    """
    if not findings:
        return

    print("[FIX] 2.2 과도한 권한 정책 조치는 운영에 큰 영향을 줄 수 있어 자동화되지 않습니다.")
