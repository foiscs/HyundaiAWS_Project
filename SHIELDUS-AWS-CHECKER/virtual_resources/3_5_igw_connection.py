...
def check():
    """
    [3.5] 인터넷 게이트웨이 연결 관리
    - 어떤 VPC에도 연결되지 않은 'detached' 상태의 인터넷 게이트웨이를 점검하고, 해당 ID 목록 반환
    """
    ...
    try:
        for igw in ec2.describe_internet_gateways()['InternetGateways']:
            # Attachments 배열이 비어 있으면 detached 상태임
...
def fix(detached_igws):
    ...
    for igw_id in detached_igws:
        choice = input(f"  -> 미사용 인터넷 게이트웨이 '{igw_id}'를 삭제하시겠습니까? (y/n): ").lower()
        if choice == 'y':
            try:
                # check 로직에서 이미 detached 상태임을 확인했으므로 바로 삭제 시도
                ec2.delete_internet_gateway(InternetGatewayId=igw_id)
                print(f"     [SUCCESS] IGW '{igw_id}'를 삭제했습니다.")
            except ClientError as e:
