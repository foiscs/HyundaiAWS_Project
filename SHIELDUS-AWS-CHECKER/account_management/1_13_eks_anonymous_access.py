...
def check():
    ...
    findings = []
    # Kubernetes가 기본적으로 생성하는 안전한 바인딩은 제외
    SAFE_BINDINGS = ["system:public-info-viewer", "kubeadm:kubelet-config-1.23"] # 버전별로 다를 수 있음
...
def fix(findings):
    """
    [1.13] EKS 익명 사용자 접근 통제 조치
    - 위험한 ClusterRoleBinding을 사용자 확인 후 삭제
    """
    ...
    for cluster_name, items in grouped_findings.items():
        ...
        if input(f"  -> 위 바인딩들을 삭제하시겠습니까? (y/n): ").lower() != 'y':
            continue
        ...
        try:
            ...
            for item in items:
                try:
                    api.delete_cluster_role_binding(name=item['binding_name'])
