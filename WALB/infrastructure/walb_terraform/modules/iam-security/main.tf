...
resource "aws_iam_user" "blog_s3_user" {
  name = "${var.project_name}-blog-s3-user"
...
resource "aws_iam_policy" "blog_s3_policy" {
  name        = "${var.project_name}-blog-s3-user-policy"
...
resource "aws_iam_policy" "splunk_kinesis_policy" {
  name        = "${var.project_name}-splunk-kinesis-reader-policy"
...
resource "aws_iam_role" "cloudtrail_cloudwatch" {
  name  = "${var.project_name}-CloudTrail-CloudWatchLogs-Role"
...
resource "aws_iam_policy" "cloudtrail_cloudwatch_policy" {
  name        = "${var.project_name}-CloudTrail-CloudWatchLogs-Policy"
  description = "${var.project_name}-CloudTrail에서 CloudWatch Logs로 전송하는 정책"
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
...
resource "aws_iam_role" "cloudwatch_kinesis" {
  name  = "${var.project_name}-CloudWatchLogs-Kinesis-Role"
  assume_role_policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
...
resource "aws_iam_policy" "cloudwatch_kinesis_policy" {
  name        = "${var.project_name}-CloudWatchLogs-Kinesis-Policy"
  description = "${var.project_name}-CloudWatch Logs에서 Kinesis로 전송하는 정책"
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
        ...
        Resource = [
          ...
          "arn:aws:kinesis:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:stream/security-hub-stream",
          "arn:aws:kinesis:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:stream/guardduty-stream",
          "arn:aws:firehose:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:deliverystream/${var.project_name}-dns-logs-to-s3"
        ]
      },
      {
        Effect = "Allow"
