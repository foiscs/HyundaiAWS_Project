...
# AWS Load Balancer Controller Helm 배포를 위한 terraform.tfvars 자동 생성 스크립트 (App2 전용)
...
# 스크립트 시작
log_info "AWS Load Balancer Controller Helm Chart 배포를 위한 terraform.tfvars 파일을 생성합니다... (App2 전용)"
...
if [ ! -d "$TERRAFORM_DIR" ]; then
    log_error "Terraform2 디렉토리를 찾을 수 없습니다: $TERRAFORM_DIR"
    log_info "현재 디렉토리: $(pwd)"
    log_info "스크립트를 helm-lbc2 디렉토리에서 실행해주세요."
    exit 1
fi

# terraform output으로부터 값 가져오기
log_info "Terraform2 인프라 정보를 가져오는 중..."
...
log_info "Terraform2 outputs 가져오는 중..."
...
PROJECT_NAME=$(terraform output -raw project_name 2>/dev/null || echo "walb2-app")
...
cat > "$TFVARS_FILE" << EOF
# AWS Load Balancer Controller Helm 배포를 위한 변수 값 (App2 전용)
# 자동 생성됨: $(date)

# 기본 정보 (terraform2 output에서 자동 수집)
project_name = "$PROJECT_NAME"
environment  = "dev"
aws_region   = "$AWS_REGION"

# EKS 클러스터 정보 (terraform2 output에서 자동 수집)
cluster_name = "$CLUSTER_NAME"
vpc_id       = "$VPC_ID"

# AWS Load Balancer Controller 설정 (App2 전용)
...
if [ -f "$TFVARS_FILE" ]; then
    ...
    echo ""
    log_warning "주의사항:"
    log_warning "- 이 파일은 자동 생성되었습니다. (App2 전용)"
    log_warning "- 필요에 따라 값들을 수정할 수 있습니다."
    log_warning "- 인프라 변경 시 이 스크립트를 다시 실행하세요."
    log_warning "- App2는 별도의 IngressClass(alb-app2)를 사용합니다."
...
log_success "스크립트 실행 완료! (App2 전용 설정)"